...
// Defualt speed and turnSpeed: 40, 50
void Balanced::Motion_Control(Direction direction, int turnSpeed=0)
{
  switch(direction)
  {
    case STOP:
    ...
    case BACK:
                  Back(30);break;
    case S_BACK_LEFT:
                  LeftThenBack(turnSpeed, 20);
                  break;
    case S_BACK_RIGHT:
                  RightThenBack(turnSpeed, 20);
                  break;
    case LEFT:
                  Left(turnSpeed);
                  break;
    case RIGHT:
                  ...
                  break;
    case SLIGHT_LEFT:
                  LeftThenFollow(turnSpeed, 7);
                  break;
    case SLIGHT_RIGHT:
                  RightThenFollow(turnSpeed, 7);
                  break;
    default:      
                  Stop();break;
  }
...
void Balanced::LeftThenBack(int turnSpeed, int speed)
{
  setting_car_speed = -speed;
  setting_turn_speed = turnSpeed;
}

void Balanced::RightThenBack(int turnSpeed, int speed)
{
  setting_car_speed = -speed;
  setting_turn_speed = -turnSpeed;
}

void Balanced::LeftThenFollow(int turnSpeed, int speed)
{
  setting_car_speed = speed;
  setting_turn_speed = turnSpeed;
}

void Balanced::RightThenFollow(int turnSpeed, int speed)
{
  setting_car_speed = speed;
  setting_turn_speed = -turnSpeed;
}
